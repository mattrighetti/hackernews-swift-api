...
extension HackerNewsScraperClient {
    // MARK: - Logout Scraping
    func scrapeLogout(
        for user: HackerNews.API.User
    ) -> String? {
        let source = try! String(contentsOf: URL(string: user.urlString)!)
        let document = try! SwiftSoup.parse(source)
        do {
            let logout = try document.getElementById("logout")
            return nil
        } catch {
            return nil
        }
